...
set infobatchname=CPUZtoText_20170731
...
REM デフォルトの結果格納ファイル名
set infoResultLog=.\SystemInformation_01_FULL.txt
...
IF "%~4"=="RECYCLE" (
 ...
 @echo.
 @echo RECYCLE指定がされたのでCPU-Zは起動せず、既得ログを利用します...
 ...
 IF NOT !ERRORLEVEL!==0 (
  @echo.
  call :colorEcho "CPU-Zの処理がうまくいかなかったようです。"
  call :colorEcho "取得済みのログがあればそれを利用して処理を続行します。"
 )

)

REM CPU-Zのログの存在チェック
IF NOT EXIST %cpuzlog% (
 @echo.
 call :colorEcho "CPU-Zのログが見つかりません。終了します。"
 set exitCode=1
 goto eof
)
...
IF NOT "%1"=="AUTO" (
 @echo.
 @echo 結果は 「%infoResultLog%」 に記録されています。
